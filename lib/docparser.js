...
YUI.add('docparser', function(Y) {
        ...
        DIGESTERS = {
            ...
            'param': function(tagname, value, target, block) {
                ...
                if (!value) {
                    ...
                    Y.log('param name/type/descript missing: ' +
                        stringlog(block), 'warn', 'docparser');
                    return;
                }

                var type, name, parts, optional, optdefault, parent,
                    multiple, len, result,
                ...
                if (!name) {
                    if (value && value.match(/callback/i)) {
                        ...
                        Y.log('Fixing missing name for callback:' +
                            stringlog(block), 'warn',
                            'docparser');
                        name = 'callback';
                        type = 'Callback';
                    } else {
                        ...
                        Y.log('param name missing: ' + value + ':' +
                            stringlog(block), 'warn',
                            'docparser');
                        name = 'UNKNOWN';
                    }
            ...
            'submodule': function(tagname, value) {
                ...
                var host = this.modules[value],
                    currentObject = this._getCurrentObject(),
                    container = this._getContainer(currentObject),
                    parent = this.get(CURRENT_MODULE);
                if (parent) {
                    host.module = parent;
                }
                if (currentObject && this[container][currentObject]) {
                    this[container][currentObject].submodule = value;
                }
                return host;
            },
            ...
            'class': function(tagname, value, target, block) {
                var namespace, fullname, host, parent, container, setter;
                ...
                setter = this._containerToSetter[container];

                block.forEach(function(def) {
                    if (def.tag === 'namespace' || def.tag === 'nested') {
                ...
                fullname = this.get(setter);
                host = this[container][fullname];
                parent = this.get(CURRENT_MODULE);
            ...
            //A key bock type for declaring interface
            'interface': 'class',

            // A key bock type for declaring custom elements
            'element': function(tagname, value, target, block) {
                ...
                Y.each(block, function(item) {
                  if (item.tag === 'interface') {
                    self.elements[name].interface = trim(item.value);
                  }
                })
            ...
            'extensionfor': function(tagname, value) {
                ...
                var container = this._getContainer(currentObject);
                if (this[container][currentObject]) {
                    this[container][currentObject].extension_for.push(value);
                }
            },
            ...
            'namespace': function(tagname, value, target, block) {
                var i = 0;
                while (i < block.length && block[i].tag !== 'class' && block[i].tag !== 'interface') {
                  i++;
                }
                if (i >= block.length) {
                  this.clearCurrentState();
                ...
                if (!this.namespaces[value].description && i >= block.length) {
                    this.namespaces[value].description = target.description;
                }
                if (value === '') {
                ...
                mod = this._getCurrentObject();
                var container = this._getContainer(mod);
                var setter = this._containerToSetter[container];
                if (mod && container !== 'namespaces') {
                    lastNS = this.get(LAST_NAMESPACE);
                    if (lastNS && lastNS !== value && (value.indexOf(
                            lastNS + '.') !== 0)) {
                        if (this[container][mod]) {
                            m = this[container][mod];
                            delete this[container][mod];
                            mod = value + '.' + mod.replace(lastNS +
                                '.', '');
                            m.name = mod;
                            m.namespace = value;
                            this[container][mod] = m;
                            this.set(setter, m.name);
                        }
                    }
                    if (this[container][mod]) {
                        this[container][mod].namespace = value;
                        if (mod === value) {
                            return;
                        }
                        if (mod.indexOf(value + '.') === -1) {
                            if (mod.indexOf('.') === -1) {
                                ...
                                m.name = name;
                                this[container][name] = m;
                                this.set(setter, name);
                            } else {
                                if (mod.indexOf(this[container][mod].namespace +
                                        '.') === -1) {
                                    m = this[container][mod];
                                    delete this[container][mod];
            ...
            'for': function(tagname, value) {
                var ns, file, mod, container, setter;
                value = trim(value);
                container = this._getContainer(value);
                setter = this._containerToSetter[container];
                this.set(setter, value);
                ns = ((this[container][value]) ?
                    this[container][value].namespace :
                    '');
            ...
            'implements': function(tagname, value) {
                var currentObject = this._getCurrentObject();
                var container = this._getContainer(currentObject);
                var setter = this._containerToSetter[container];

                if (currentObject) {
                    if (!this[container][currentObject].implements) {
                        this[container][currentObject].implements = [];
                    }
                    this[container][currentObject].implements.push(value);
                    this.set(setter, currentObject);
                }
            },
            'enum': function(tagname, value, target, block) {
                var items = REGEX_TYPE.exec(implodeString(trim(value)));

                target.itemtype = tagname;
                target.name = items[2];
                target.description = trim(items[3]);
                target.type = 'Enum';
                target.values = [];

                Y.each(block, function(elem) {
                    if (elem.tag === 'value') {
                        items = REGEX_TYPE.exec(implodeString(trim(elem.value)));

                        target.values.push({
                            name: items[2],
                            description: trim(items[3])
                        })
                    }
                });
            },
            'kind': function(tagname, value, target) {
                var items = REGEX_TYPE.exec(implodeString(trim(value)));
            ...
            'fires': function(tagname, value, target) {
                var desc = implodeString(trim(value)),
                    type,
                    kind,
                    match = REGEX_TYPE.exec(desc),
            ...
            'nested': function(tagname, value) {
              var valueContainer = this._getContainer(value);
              var currentObject = this._getCurrentObject();
              var container = this._getContainer(currentObject);
              if (valueContainer) {
                if (!this[valueContainer][value].nested) {
                  this[valueContainer][value].nested = [];
                }
                this[valueContainer][value].nested.push(currentObject);
                this[container][currentObject].nestedOf = value;
              }
            }
        },
    ...
    DocParser.ATTRS = {
            ...
            setter: function(o) {
                ...
                if (this.get(CURRENT_MODULE) === name) {
                    ...
                    if (name in this.modules) {
                        ...
                        if (write) {
                            //console.log('Writing');
                            this.modules[name] = Y.merge(this.modules[
                                name], o);
                        }
                    } else {
                        if (o._main) {
        ...
        currentmodule: {
            setter: function(val) {
                ...
                var container = this._getContainer(currentObject);
                if (container) {
                    var obj = this[container][currentObject];
                }
                if (obj) {
                    //Handles case where @module comes after @class in a new directory of files
                    if (obj.module !== val) {
                        if (this.modules[obj.module]) {
                            delete this.modules[obj.module].submodules[obj.submodule];
                            delete this.modules[obj.module][container][obj.name];
                        }
                        if (obj.submodule && this.modules[obj.submodule]) {
                            delete this.modules[obj.submodule].submodules[obj.submodule];
                            delete this.modules[obj.submodule][container][obj.name];
                        }
                        obj.module = val;
                        if (this.modules[val]) {
                            this.modules[val].submodules[obj.submodule] = 1;
                            this.modules[val][container][obj.name] = 1;
                        }
                        if (obj.submodule && this.modules[obj.submodule]) {
                            this.modules[obj.submodule].module = val;
                        }
                    }
                }
        ...
        currentnamespace: {
            setter: function(val) {
                ...
                if (val.lastIndexOf('.') > -1) {
                    var parent = val.slice(0, val.lastIndexOf('.'));
                    if (!(parent in this.namespaces)) {
                    ...
                    if (this.namespaces[parent].namespaces.indexOf(val) == -1) {
                      this.namespaces[parent].namespaces.push(val);
                    }
                }
                this.set(LAST_NAMESPACE, this.get(CURRENT_NAMESPACE));
                return val;
            }
        },
        lastnamespace: {
        ...
        /**
         * The minterface currently being parsed
         * @attribute currentinterface
         * @type String
         */
        currentinterface: {
            setter: function(val) {
                ...
                this.set('lastinterface', this.get(CURRENT_INTERFACE));
                val = trim(val);
                var name = val,
                    ns, minterface;
                if (!(val in this.interfaces)) {
                    ns = this.get(CURRENT_NAMESPACE);
                    if (ns && ns !== '' && (val.indexOf(ns + '.') !== 0)) {
                        name = ns + '.' + val;
                    }
                    minterface = this.interfaces[name] = {
                        name: name,
                        shortname: val,
                        items: [],
                        plugins: [],
                    ...
                    minterface.module = this.get(CURRENT_MODULE);
                    if (this.get(CURRENT_SUBMODULE)) {
                        minterface.submodule = this.get(
                            CURRENT_SUBMODULE);
                    }
                    if (ns && ns !== '') {
                        minterface.namespace = ns;
                        this.namespaces[ns].interfaces.push(minterface.name);
                    }
                }
                this.set(CURRENT_CLASS, '');
                return name;
            }
        },
        lastinterface: {
        ...
        currentclass: {
            setter: function(val) {
                ...
                if (!(val in this.classes)) {
                    ...
                    if (ns && ns !== '') {
                        clazz.namespace = ns;
                        this.namespaces[ns].classes.push(clazz.name);
                    }
                }
                return name;
            }
    ...
    Y.extend(DocParser, Y.Base, {
        ...
        clearCurrentState: function(){
          ...
          this.set(CURRENT_SUBMODULE, '');
          this.set(CURRENT_CLASS, '');
          this.set(CURRENT_ELEMENT, '');
          this.set(CURRENT_INTERFACE, '');
        },
        initializer: function() {
            ...
            self.after('currentclassChange', function(e) {
                var clazz = e.newVal;
                Y.each(self.classitems, function(
                    item) {
                    if (!(item.class)) {
                        item.class = clazz;
                    }
            ...
            self.after('currentinterfaceChange', function(e) {
                var minterface = e.newVal;
                Y.each(self.interfaceitems, function(item) {
                    if (!item.interface) {
                        item.interface = minterface;
                    }
                })
            })
        },
        ...
        _getCurrentObject: function() {
            var current = this.get(CURRENT_CLASS) || this.get(CURRENT_INTERFACE);
            if (current) {
                return current;
            }
            var currentNamespace = this.get(CURRENT_NAMESPACE);

            return currentNamespace && currentNamespace !== '' ? currentNamespace : undefined;
        },
        _getContainer: function(obj) {
            for (var i = 0; i < this.containers.length; i++) {
                if (this[this.containers[i]][obj]) {
        ...
        **/
        ...
        extract: function(filemap, dirmap) {
            ...
            Y.each(filemap, function(code, filename) {
                ...
                for (i = 0; i < len; i++) {
                    line = lines[i];
                    if (REGEX_START_COMMENT[
                            syntaxtype].test(line)) {
                        commentlines = [];
        ...
        processblock: function(block) {
                ...
                if (tag && tag.tag) {
                    if (!(name in this.knowntags)) {
                        if (name in CORRECTIONS) {
                            ...
                            Y.log('replacing incorrect tag: ' + name + ' with ' + CORRECTIONS[name] + ': ' + stringlog(block),
                                'warn',
                                'docparser');
                            ...
                            Y.log('unknown tag: ' + name + ',' + stringlog(block),
                                'warn',
                                'docparser');
                        }
                    }
            ...
            } else if ((currentObject = this._getCurrentObject()) !== undefined) {
                var container = this._getContainer(currentObject);

                this[container][currentObject].items.push(target);
        ...
        transform: function(commentmap) {
            ...
            Y.each(classes, function(clazz, name) {
                self._transformObject('classes', classes, clazz, name, modules, files);
            });

            Y.each(interfaces, function(minterface, name) {
                self._transformObject('interfaces', interfaces, minterface, name, modules, files);
            })
        ...
        _transformObject: function(container, elements, obj, name, modules, files) {
            var self = this;
            if (obj.module) {
                modules[obj.module][container][name] = 1;
            }
            if (obj.submodule) {
                modules[obj.submodule][container][name] = 1;
                if (!modules[obj.submodule].description) {
                    modules[obj.submodule].description = obj.description;
                }
            }

            if (obj.file) {
                files[obj.file][container][name] = 1;
                if (modules[obj.module]) {
                    modules[obj.module].file = obj.file;
                    modules[obj.module].line = obj.line;
                }
                if (modules[obj.submodule]) {
                    modules[obj.submodule].file = obj.file;
                    modules[obj.submodule].line = obj.line;
                }
            }
            if (obj.uses && obj.uses.length) {
                obj.uses.forEach(function(u) {
                    var c = elements[u];
                    if (c) {
                        c.extension_for.push(obj.name);
                    }
                });
            }
            Y.each(obj.items, function(v) {
                if (!v.itemtype) {
                    self.warnings.push({
